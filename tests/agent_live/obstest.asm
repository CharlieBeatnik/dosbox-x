; obstest.asm  --  DOSBox-X agent observability live-fire test.
;
...
; commands added:
;
;   debug.status    hit counters on BPs and watches; bytes_now.
;   cpu.probe       non-halting execution counters.
;   cpu.trace_ring  + cpu.traceback — "how did we get here?".
;   cpu.disasm      structured disassembly of known bytes.
;
...
;
;   '1'  Scenario 1 (probe / hit counters): run a known, bounded NOP loop body
...
;        same address (separate sub-scenario) must report the same hit count.
;
;   '2'  Scenario 2 (disasm): does nothing but spin back to the loop. The bytes
...
;
;   '3'  Scenario 3 (trace ring): execute a short, distinctive straight-line
...
;
;   '4'  Scenario 4 (mem.watch): a single, known store —
...
;        new==WATCH_VALUE. This is the "name the stamp site"
;        test made concrete. Run once (the field stays WATCH_VALUE after).
;
;   '5'  Scenario 5 (mem.watch on VGA framebuffer): switch to planar mode 12h,
...
;        This is the "VGA blind spot" closure made concrete.
;
;   '6'  Scenario 6 (multi-sentinel cpu.watch_target): two NEAR calls to
...
;        debug.status. Proves the multi-sentinel `which` field.
;
;   '7'  Scenario 7 (cpu.step / cpu.step_over): a single known NEAR call.
...
landmarks_table:
        ...
        dw      landmark_store          ; +14 the storing instruction (scenario 4)
        dw      landmark_vga_store      ; +16 the VGA storing instruction (scenario 5)
        dw      landmark_msa            ; +18 multi-sentinel target A (scenario 6)
        dw      landmark_msb            ; +20 multi-sentinel target B (scenario 6)
...
main_loop:
        ...
        cmp     al, '4'
        jne     chk_5
        jmp     scenario4                  ; near jmp: scenario4 is out of je's rel8 range
chk_5:
        cmp     al, '5'
        jne     chk_6
        jmp     scenario5                  ; near jmp: scenario5 is out of je's rel8 range
chk_6:
        cmp     al, '6'
        jne     chk_7
...
; ---- Scenario 1: bounded NOP loop (probe + hit counters) --------------------
...
; ---- Scenario 2: disasm fodder (never executed) -----------------------------
...
; ---- Scenario 3: distinctive straight-line chain (trace ring) ---------------
...
; ---- Scenario 4: single known store (mem.watch write-intercept) -------------
...
; ---- Scenario 5: VGA framebuffer write (mem.watch VGA blind-spot) -------
...
; ---- Scenario 6: multi-sentinel cpu.watch_target ------------------------
...
; ---- Scenario 7: known NEAR call (cpu.step / cpu.step_over) -------------
